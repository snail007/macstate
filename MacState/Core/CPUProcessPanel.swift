...
@MainActor
final class CPUProcessPanel: NSObject, NSTableViewDataSource, NSTableViewDelegate {
    static let shared = CPUProcessPanel()

    private var panel: NSPanel?
    private var tableView: NSTableView?
    private var summaryLabel: NSTextField?
    private var processes: [ProcessCPUUsage] = []
    private var refreshTimer: Timer?
    private var detailPanel: NSWindow?
    private var lastCpuUsage: String = ""

    private let iconColumnID = NSUserInterfaceItemIdentifier("cpuIcon")
    private let pidColumnID = NSUserInterfaceItemIdentifier("cpuPid")
    private let nameColumnID = NSUserInterfaceItemIdentifier("cpuName")
    private let usageColumnID = NSUserInterfaceItemIdentifier("cpuUsage")
    private let commandColumnID = NSUserInterfaceItemIdentifier("cpuCommand")

    private override init() {
        super.init()
    }

    func toggle(cpuUsage: String) {
        if let p = panel, p.isVisible {
            stopRefreshTimer()
            p.orderOut(nil)
            return
        }
        show(cpuUsage: cpuUsage)
    }

    private func show(cpuUsage: String) {
        ...
        panel.title = "\(L10n.shared.moduleName(.cpuUsage)) — \(L10n.shared.topProcesses)"
        updateColumnTitlesForLanguage()
        summaryLabel.stringValue = "\(L10n.shared.moduleName(.cpuUsage)): \(cpuUsage)"

        DispatchQueue.global(qos: .userInitiated).async {
            let result = ProcessCPUService.shared.topProcesses(limit: 10)
    ...
    private func refreshData() {
        ...
        let cpuValue = MonitorManager.shared.cpuUsage
        lastCpuUsage = String(format: "%.1f%%", cpuValue)
        summaryLabel?.stringValue = "\(L10n.shared.moduleName(.cpuUsage)): \(lastCpuUsage)"
    ...
    private func buildPanel() {
        ...
        p.title = "\(L10n.shared.moduleName(.cpuUsage)) — \(L10n.shared.topProcesses)"
        ...
        table.autosaveName = "CPUProcessTable"
        ...
        self.panel = p
        p.setFrameAutosaveName("CPUProcessPanel")
    }
    ...
    nonisolated func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        return MainActor.assumeIsolated {
            ...
            if columnID == usageColumnID {
                let cell = NSTextField(labelWithString: String(format: "%.1f%%", proc.cpuPercent))
